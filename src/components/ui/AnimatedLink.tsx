...
function StyledArrow({ size = "medium" }: { size?: "small" | "medium" | "large" }) {
    const sizes = {
...
export function AnimatedLink({ href, children, className = "", onClick, arrowSize }: AnimatedLinkProps) {
    ...
    const getArrowSize = (): "small" | "medium" | "large" => {
        ...
        if (className.includes("text-xl") || className.includes("text-sm") || className.includes("text-base")) {
            return "small";
        }
